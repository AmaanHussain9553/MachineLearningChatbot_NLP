...
def welcome_state():
    ...
    print("Hello there! My name is Sayn. I am a chatbot created by a word class computer scientist, Amaan Hussain"
          "\nI can perform the following tasks:"
          "\n\t\t 1.Perform a preliminary health care check"
          "\n\t\t 2.Analyse your text sentences and determine the stylistic features"
          "\n\nLet's get started!!!! I am excited, aren't you ?")
    return ""
...
def get_info_state():
    ...
    user_input = input("Let's go ahead and start basics,shall we ?"
                       "\nWhat is your name and date of birth?"
                       "\nEnter in the format First_name Last_name MM/DD/YY"
                       "\nFor example:"
                       "\n\t\tJohn Doe 08/16/01")
...
def stylistic_analysis_state():
    ...
    print(
        "Thanks!  Based on my stylistic analysis, I've identified the following psychological correlates in "
        "your response:")
...
def check_next_state():
    next_state = input("What would you like to do next?"
                       "\n\t1.I can redo the health check if that what you would like to do"
                       "\n\t2.I can go over the stylistic analysis analysis again"
                       "\n\t3.You can say farewell to me and terminate this chatbot :("
                       "\nPlease type in 1, 2, or 3 as your choice\n")
    return next_state
...
def run_chatbot(model, word2vec):
    ...
    welcome_state()
    get_info_state()
    health_check_state(model, word2vec)
    stylistic_analysis_state()
    user_next_move = check_next_state()
    while user_next_move != "3":
        if user_next_move == "1":
            health_check_state(model, word2vec)
        elif user_next_move == "2":
            stylistic_analysis_state()
        user_next_move = check_next_state()
    print("Hope you had a great experience with Sayn, our intelligent chatbot. Hope you have a great day")
    return
